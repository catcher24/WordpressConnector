...
export const setupStorybookMocks = (customMocks: any = {}) => {
  ...
  window.fetch = async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (url.includes("/targets") && !url.includes("/vulnerabilities") && !url.includes("/scans") && !url.includes("/certificates") && !url.includes("/rootDomains") && !url.includes("/ports") && !url.includes("/select")) {
      if (url.match(/\/targets\/[a-zA-Z0-9-]+$/)) {
        return respondWith(customMocks.target || generateMockTarget({ id: "target-123", hostname: "example.com", preferredDisplayName: "My WordPress Site" }));
      }
      return respondWith({ items: customMocks.targets || [generateMockTarget({ id: "target-123", hostname: "example.com", preferredDisplayName: "My WordPress Site" }), generateMockTarget()] });
    }

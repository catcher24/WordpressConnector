 ...
 * API Route
 *
 * @package Catcher24WordpressConnector\API
 * @since 1.0.0
 */
...
class Route {
   ...
   * @param bool|string|callable|null $auth auth callback.
   *
   * @return array
   */
  ...
  public static function dispatch() {
    foreach ( self::$routes as $prefix ) {
      foreach ( $prefix as $route ) {
        ...
        $route      = (object) $route;
        $route_auth = $route->auth ?? false;

        $callback = self::prepare_callback( $route->callback );
        $auth     = self::prepare_callback( $route_auth );
